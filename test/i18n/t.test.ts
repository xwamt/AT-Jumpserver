...
describe('t', () => {
  ...
  it('returns a message that has no placeholders unchanged', () => {
    expect(t('Configure JumpServer')).toBe('Configure JumpServer');
    expect(t('Configure JumpServer', { unused: 'ignored' })).toBe('Configure JumpServer');
  });
  ...
  it('substitutes falsy numbers and booleans instead of dropping to the placeholder', () => {
    expect(t('{count} items, verifyTls: {verifyTls}', { count: 0, verifyTls: false })).toBe(
      '0 items, verifyTls: false'
    );
  });

  it('leaves a placeholder with no matching argument literal', () => {
    expect(t('JumpServer at {url} ({version})', { url: 'https://jumpserver.local' })).toBe(
      'JumpServer at https://jumpserver.local ({version})'
    );
  });
});
