...
describe('package.nls files', () => {
  it('declares exactly the same keys in both languages', () => {
    ...
    // raw `%atJumpServer.whatever%` placeholder in the language that is missing it,
  ...
  it('namespaces every key under atJumpServer', () => {
    ...
    for (const key of Object.keys(english)) {
      expect(key.startsWith('atJumpServer.'), key).toBe(true);
    }
  });
});
